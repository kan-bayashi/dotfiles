...
function my_git_prompt_info() {
  ...
  if [ "${ref#refs/heads/}" = "master" ];then
    echo "$ZSH_THEME_GIT_PROMPT_MASTER_PREFIX${ref#refs/heads/} $(parse_git_dirty)$ZSH_THEME_GIT_PROMPT_SUFFIX"
  else
    echo "$ZSH_THEME_GIT_PROMPT_BRANCH_PREFIX${ref#refs/heads/} $(parse_git_dirty)$ZSH_THEME_GIT_PROMPT_SUFFIX"
  fi
}
...
if [ ! -n "${BULLETTRAIN_GIT_PROMPT_CMD+1}" ]; then
  BULLETTRAIN_GIT_PROMPT_CMD="\$(my_git_prompt_info)"
fi
...
if [ ! -n "${BULLETTRAIN_GIT_MODIFIED+1}" ]; then
  ZSH_THEME_GIT_PROMPT_MODIFIED="%F{green}󰺫 %F{black}"
else
...
if [ ! -n "${BULLETTRAIN_GIT_UNTRACKED+1}" ]; then
  ZSH_THEME_GIT_PROMPT_UNTRACKED="%F{yellow}󱕣 %F{black}"
else
